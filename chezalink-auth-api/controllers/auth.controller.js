...
const generateToken = (user) => {
  return jwt.sign(
     {
      id: user.user_name,           // can keep for backward compatibility
      email: user.email,
...
const signup = async (req, res) => {
  ...
  try {
    ...
    const newUserResult = await pool.query(
      'INSERT INTO accounts (user_name, email, password) VALUES ($1, $2, $3) RETURNING user_name, email',
...
const getAllUsers = async (req, res) => {
  try {
    const result = await pool.query('SELECT user_name, email FROM accounts');
